...
- 连接状态实时同步，基于Stream的连接状态监控
- 设备详情页面协议通信监控，支持DYJV2设备的完整协议栈
- 优化的设备卡片设计，突出显示连接状态和信号强度

**UI/UX改进**:
- 移除已连接设备卡片的断开按钮，避免误操作
- 只保留详情按钮，用户需进入详情页面才能断开连接
- 设备列表自动刷新机制，确保连接状态信息最新
- 简洁的已连接设备展示，包含设备类型、信号强度、协议状态
