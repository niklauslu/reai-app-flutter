...
# 生成CSR文件和私钥
openssl req -new -newkey rsa:2048 -nodes -keyout private.key -out development_certificate.csr -subj "/emailAddress=$email/CN=$common_name"

if [ $? -eq 0 ]; then
    echo ""
    echo "✅ CSR文件生成成功!"
    echo "📁 文件位置:"
    echo "   CSR文件: $(pwd)/development_certificate.csr"
    echo "   私钥文件: $(pwd)/private.key"
    echo ""
    echo "📋 下一步:"
    ...
    echo "6. 上传 development_certificate.csr 文件"
    echo "7. 下载并安装生成的证书"
    echo ""
